package JMSTopicSubscriberDemo.activemq;
...
public static void main(String[] args) {
	...
	// 开启5个线程，不断接收消息
	for (int i = 0; i < 5; i++) {
		String consumerName = "JMSTopicSubscriber_" + String.valueOf(i);
		Thread t = new Thread(new Runnable() {
			public void run() {
				Connection connection = null;// 连接

				Session session;// 会话 接受或者发送消息的线程
				Destination destination;// 消息的目的地 ，其实就是连接到哪个队列，如果是点对点，那么它的实现是Queue，如果是订阅模式，那它的实现是Topic
